...
Após todos os pontos de interesse estarem visitados ou não existirem condições para visitar os que ainda não estão, o GPS encaminha o Driver para o ponto final da viagem, situação que também se verifica aquando da viagem direta em que são ignorados os pontos de interesse e o Driver é encaminhado diretamente para o ponto final. \\

A escolha de caminho feita pelo GPS pode ser interrompida de duas formas: o agente Driver avista um acidente no seu raio de visão ou o agente Rádio comunica através de mensagem a existência de um acidente no mapa de estradas. Em ambos os casos a estratégia seguida pelo GPS é idêntica: marca no seu mapa de estradas o acidente (ficando assim o acidente referenciado e sendo considerado em futuros cálculos) e verifica a necessidade de recalcular a rota do Driver. O procedimento para recalcular a rota é idêntico ao já explicado, visto que o acidente foi marcado no mapa pelo GPS e portanto todos os caminhos que passem pelo menos não serão considerados. \\

\subsection{Driver}

O agente Driver importa a capacidade GPS para fazer uso do seu conhecimento tal como acontece numa situação real de utilização de um sistema de GPS.
...
O objetivo principal do agente passa por analisar o ambiente circundante (raio de visão) verificando a existência de acidentes. Caso seja avistado um acidente este é marcado no mapa e o GPS que irá recalcular a nova rota a seguir.
...
\item{\textit{IBDIExternalAccess} - interface que permite aceder às propriedades de um agente fora do seu âmbito.}
\end{itemize}
...
\begin{lstlisting}
...
final IVector2 destination = (IVector2) getProperty(PROPERTY_DESTINATION); //posição do destino final
final IVector2 interdestination = new Vector2Double(path.get(0).x, path.get(0).y); //posição da célula intermédia adjacente
double speed = ((Number) obj.getProperty(PROPERTY_SPEED)).doubleValue(); //valor definido para a velocidade
double maxdist = progress * speed * 0.001; //distância máxima que pode percorrer, tendo em conta o progresso do deslocamento atual
IVector2 loc = (IVector2) obj.getProperty(Space2D.PROPERTY_POSITION); //posição atual do condutor

IVector2 newloc = ((Space2D) space).getDistance(loc, interdestination).getAsDouble() <= maxdist? interdestination : interdestination.copy().subtract(loc).normalize().multiply(maxdist).add(loc); //nova posição baseada na posição atual, posição final e distância máxima que pode percorrer

((Space2D) space).setPosition(obj.getId(), newloc); //movimento para a nova localização
...
\begin{lstlisting}
...
final IBDIExternalAccess agent = (IBDIExternalAccess) getProperty(PROPERTY_SCOPE); //acedendo ao scope da task é possível criar um acesso externo ao agente, pois foi este que iniciou a task
...
agent.scheduleStep((IComponentStep) new IComponentStep<Void>() {
 public IFuture<Void> execute(IInternalAccess ia) {
  IBDIInternalAccess bia = (IBDIInternalAccess) ia; //acesso interno do agente
  ...
  bia.getBeliefbase().getBelief("acidente").setFact(so); //acede-se ao belief acidente e atualiza-se com o novo acidente encontrado
  ...
  return IFuture.DONE;
 }
});
