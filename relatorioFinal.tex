...
São assim eliminados \textit{a priori} os pontos de interesse não válidos por um qualquer motivo.
...
A escolha de caminho feita pelo \textit{GPS} pode ser interrompida de duas formas: o agente \textit{Driver} avista um acidente no seu raio de visão ou o agente Rádio comunica através de mensagem a existência de um acidente no mapa de estradas. Em ambos os casos a estratégia seguida pelo \textit{GPS} é idêntica: marca no seu mapa de estradas o acidente (ficando assim o acidente referenciado e sendo considerado em futuros cálculos) e verifica a necessidade de recalcular a rota do \textit{Driver}. O procedimento para recalcular a rota é idêntico ao já explicado, visto que o acidente foi marcado no mapa pelo \textit{GPS} e portanto todos os caminhos que passem pelo menos não serão considerados. \\

\subsection{\textit{Driver}}

O agente \textit{Driver} importa a capacidade \textit{GPS} para fazer uso do seu conhecimento tal como acontece numa situação real de utilização de um sistema de \textit{GPS}. \\

Num determinado momento o \textit{Driver} tem conhecimento do ponto inicial da viagem, do ponto final e do ponto de interesse para onde se está a deslocar para além de saber se está a realizar uma viagem do tipo turística ou direta. \\
...
\subsection{\textit{Radio}}
 
O agente \textit{Radio} funciona como um rádio real, avisando o condutor de todos os acidentes que existem no mapa. O agente tem como conhecimento o próprio mapa, estando assim ciente de todos os acidentes, quer sejam os iniciais, quer os que são adicionados em \textit{runtime}. \\

O \textit{Radio} comunica através de eventos por mensagem com o \textit{Driver}, indicando numa primeira comunicação todos os acidentes e posteriormente aquando do aparecimento de novos acidentes, enviando uma nova comunicação em que são apenas enviados os novos acidentes.

\section{Aplicação}

A aplicação é constituída pelos seguintes elementos:
...
O agente Driver importa a capacidade GPS para fazer uso do seu conhecimento e como tal interage com ela através dos seus \emph{beliefs} e \emph{goals}. Esta estrutura e interação tenta ser fiel à interação real que existe um sistema de GPS e um condutor humano. O condutor pode consultar o conhecimento do GPS e vai, ao longo da viagem, inserindo novos dados no sistema que os tratará devidamente. \\

O agente Rádio por sua vez comunica com o \textit{Driver} através de eventos por mensagem, enviando mensagens que possuem informação sobre os acidentes presentes no espaço. Mais uma vez, esta interação é o mais fiel possível a uma situação real em que o condutor ouve uma mensagem proveniente do rádio e insere a informação recebida no \textit{GPS} sendo que este recalculará a nova rota tendo em conta os novos dados. \\

Quer o Rádio quer o \textit{GPS} interagem com a Aplicação através da \textit{Grid2D} onde estão contidos os vários elementos da simulação. Desta forma o Rádio tem conhecimento dos acidentes, assim como o \textit{Driver} e o \textit{GPS} possuem informação sobre os vários pontos de interesse a visitar e os diferentes caminhos possíveis. \\
...
Esta experiência tem como objetivo testar o algoritmo implementado, o A*. Analisando o mapa anterior conseguimos ver que a Igreja é o primeiro ponto a ser visitado pois é o que se encontra mais próximo da partida tal como demonstram os \textit{logs} das ações.

Analisando os \textit{logs} de um percurso completo e o mapa do mundo, vemos que o condutor percorre corretamente todos os pontos de interesse antes de chegar ao ponto final, tal como pretendido.
...
Tendo o estado do tempo definido com "Sol", e sendo que o Zoo só é visitado quando está sol o condutor terá que visitá-lo. Nesta primeira experiência foi possível comprovar que tal acontece.
...
\begin{figure}[H]
  ...
  \caption{Ações do condutor em percurso direto.}
\end{figure}

Tal como anteriormente, verificamos que o condutor processa de forma correta a informação relativa a um acidente, independentemente de esta ter vindo do rádio ou do seu raio de visão.
...
Em termos de melhoramentos possíveis podemos destacar alguns que poderiam ter sido implementados caso o tempo de desenvolvimento fosse um pouco mais alargado. \\

Em primeiro lugar a inserção dinâmica de pontos de interesse, gestão das respetivas pré-condições e gestão do ponto inicial e final da viagem permitiria ao utilizador ter um maior controlo sobre a simulação para além das interfaces já implementadas. \\

Em segundo, a adaptação das interfaces disponíveis para alojarem vários condutores seria uma melhoria que permitiria lançar a aplicação para um novo nível. Tal como foi referido no relatório, a estrutura da aplicação foi já adaptada para este cenário e é possível observar a respetiva simulação com dois condutores. No entanto as interfaces disponíveis não estão prontas para este cenário e não é assim possível controlar facilmente todos os aspetos. \\
...
\begin{itemize}
\item Bruno Ferreira: 20\% 
\item Carlos Babo: 40\%
\item Hélder Moreira: 40\% 
\end{itemize}

\chapter{Apêndice}
\section{Manual do utilizador}

Abrir a linha de comandos e executar o comando \textit{"java -jar DriverAssistant.jar"}.
Ao correr a aplicação abrirá a janela do \textit{Jadex Control Center}, onde é necessário adicionar ao \textit{path} o \textit{jar} do ficheiro. \\

Abrir a pasta "\textit{application}", selecionar "\textit{BDIDriveAssistant.application.xml}" e depois consoante o pretendido escolher uma das três opções na "\textit{Configuration}" e carregar no botão \textit{Start}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{start}
  \caption{Janela Principal do \emph{Jadex Control Center}.}
\end{figure}

Na interface principal é possível iniciar a simulação, ligar e desligar o rádio, bem como adicionar e remover acidentes e caminhos no mapa. Para isso deverá clicar numa célula com o botão esquerdo e ela avançará para o seu próximo estado. Existem três estados possíveis: caminho, acidente e caminho indisponível que são identificados através de diferentes cores (vermelho = acidente, cinza claro = caminho indisponível e cinza escuro = caminho disponível).
...
É possível ver os \emph{beliefs} do condutor e as suas ações na janela do log das ações do condutor.
...
A última janela, \textit{BDI Drive Assistant}, mostra os diferentes pontos de interesse e acidentes do percurso, o ponto inicial e final, bem como o condutor a deslocar-se ao longo do mapa.
