...
O agente Rádio por sua vez comunica com o Driver através de eventos por mensagem, enviando mensagens que possuem informação sobre os acidentes presentes no espaço. Mais uma vez, esta interação é o mais fiel possível a uma situação real em que o condutor ouve uma mensagem proveniente do rádio e insere a informação recebida no GPS sendo que este recalculará a nova rota tendo por os novos dados. \\

Quer o Rádio quer o GPS interagem com a Aplicação através da Grid2D onde estão contidos os vários elementos da simulação. Desta forma o Rádio tem conhecimento dos acidentes, assim como o Driver e o GPS possuem informação sobre os vários pontos de interesse a visitar e os diferentes caminhos possíveis. \\
...
Abrir o \emph{IDE} e escolhe como main class "jadex.base.Starter".
Ao correr a aplicação abrirá a janela do Jadex Control Center, onde é necessário adicionar ao path a pasta "application" que se encontra dentro da pasta src do projeto.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{addPath}
  \caption{Janela do \emph{Path} do \emph{Jadex Control Center}.}
\end{figure}
